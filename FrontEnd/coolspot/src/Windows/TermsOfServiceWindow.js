...
function TermsOfServiceWindow() {
    ...
    const { visible } = windowStates.termsOfServiceWindow || {};

    const handleClose = () => {
        updateWindowState('termsOfServiceWindow', { visible: false });
    };

    return (
        <div
            id="modal-overlay"
            className={`fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center z-40 p-4 sm:p-6
        ...
        >
            <div className="bg-white p-6 rounded-lg shadow-lg 
                ...
                <h2 className="text-2xl font-bold text-center text-gray-800 mb-4">Terms of Service</h2>

                <div className="prose max-h-[60vh] overflow-y-auto">
                    {window.websiteSetting?.TOS && (
                        <div dangerouslySetInnerHTML={{ __html: window.websiteSetting.TOS }} />
                    )}
                </div>
            </div>
        </div>
