...
function AddSpotWindow() { 
  ...
  return (
    <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-lg shadow-lg z-100 
        w-11/12 sm:w-5/6 md:w-4/5 lg:w-1/2 xl:w-1/3" 
        style={{ zIndex: 100 }}>
      <button className="absolute top-0 right-0 w-8 h-8 rounded-tr-lg rounded-bl-lg text-2xl bg-red-600 text-white font-bold flex items-center justify-center hover:bg-red-500" style={{ width: '30px', height: '30px' }} onClick={onClose}>
        &times;
      </button>
      <h2 className="text-2xl font-bold mb-4 text-center">Add New Spot</h2>
      
      <input type="text" className="w-full p-2 border-b-2 border-gray-300 focus:outline-none focus:ring-0 focus:border-blue-500 transition-colors duration-300 mb-1" 
        placeholder="Spot Name" 
        value={spotName} 
        onChange={(e) => { setSpotName(e.target.value); setErrorMessage(''); }} 
      />

      {/* Non-resizable description box with scrollability */}
      <textarea className="w-full p-2 border-b-2 border-gray-300 focus:outline-none focus:ring-0 focus:border-blue-500 transition-colors duration-300 mb-4" 
      ...
      <div className="overflow-x-auto mb-4 flex items-center space-x-4">
        <div className="flex flex-nowrap gap-4">
          ...
          {/* Add Images button with blue border */}
          <div className="border-2 border-blue-500 rounded-lg p-4 text-center cursor-pointer flex-none" style={{ width: '100px', height: '100px' }}>
            <input type="file" multiple className="hidden" id="image-upload" onChange={handleImageChange} />
      ...
      <div className="flex justify-end mt-4">
        <button onClick={onClose} className="mr-4 bg-gray-300 text-black px-4 py-2 rounded-lg shadow hover:bg-gray-400 transition-colors">Cancel</button>
        <button onClick={handlePublishSpot} className="bg-blue-600 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-700 transition-colors">Publish Spot</button>
      </div>
    </div>
  );
}
